...
import { getCachedPetsBase } from '@/data-access/pet'
...
export const PetList = async () => {
  ...
  const pets = await getCachedPetsBase()
    ...
    <ul className="grid gap-x-3 gap-y-3 overflow-hidden sm:gap-x-5 sm:gap-y-5 md:grid-cols-2 2xl:grid-cols-3">
      {pets.map((pet, index) => (
        <li key={index} className="overflow-hidden">
          <PetCard id={pet.id} name={pet.name} slug={pet.slug} avatarSrc={pet.photos[0].src} />
        </li>
      ))}
    </ul>
  )
